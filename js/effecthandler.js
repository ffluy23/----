// effecthandler.js (더블배틀 서버용)
// 상태이상 값: "독" / "화상" / "마비" / "얼음" (한국어 통일)
...
// 씨뿌리기 EOT 흡수
export function applyLeechSeed(seedeeEntry, seedeeIdx, seederEntry, seederIdx) {
  const seedee = seedeeEntry[seedeeIdx]
  const seeder = seederEntry[seederIdx]
  if (!seedee || seedee.hp <= 0 || !seedee.seeded) return []
  const dmg = Math.max(1, Math.floor((seedee.maxHp ?? seedee.hp) / 16))
  seedee.hp = Math.max(0, seedee.hp - dmg)
  const msgs = [`${seedee.name}${josa(seedee.name, "은는")} 씨뿌리기로 ${dmg} 데미지를 입었다!`]
  if (seeder && seeder.hp > 0) {
    const heal = Math.min(dmg, (seeder.maxHp ?? seeder.hp) - seeder.hp)
    if (heal > 0) {
      seeder.hp += heal
      msgs.push(`${seeder.name}${josa(seeder.name, "은는")} 체력을 흡수했다! (+${heal})`)
    }
  }
  if (seedee.hp <= 0) msgs.push(`${seedee.name}${josa(seedee.name, "은는")} 쓰러졌다!`)
  return msgs
}

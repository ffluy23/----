// js/doublebattle.js — 인트로는 doubleintro.js로 분리됨
...
import { initIntro, skipIntro, fadeBgmOut } from "./doubleintro.js"
...
function onMoveClick(idx, moveInfo, data) {
  ...
  if (targetsEnemy) enterTargetMode(idx, data)
  else doUseMove(idx, [], data)
}
...
async function doForcedSwitch(newIdx) {
  try { await _forcedSwitch({ roomId: ROOM_ID, mySlot, newIdx }) }
  catch(e) { console.error("forcedSwitch 오류:", e.message) }
}
...
function updateTurnUI(data) {
  ...
  if (isSpectator) {
    if (order.length > 0) {
      const s = order[0]
      const name = (data[`${s.replace("p","player")}_name`] ?? s).split("]").pop().trim()
  ...
  if (pending.includes(mySlot)) {
    ...
    el.innerText = idx > 0 ? `${idx}번째 대기중...` : "상대 턴..."; el.style.color = "gray"
  }
...
async function doSkipTurn() {
  try { await _skipTurn({ roomId: ROOM_ID, mySlot }) }
...
function showAssistAnimation() {
  return new Promise(resolve => {
    ...
    el.classList.remove("assist-show"); void el.offsetWidth
...
function showSyncAnimation() {
  return new Promise(resolve => {
    const el = $("sync-anim")
    if (!el) { resolve(); return }
    el.classList.remove("sync-show"); void el.offsetWidth
...
function listenRoom() {
  onSnapshot(roomRef, async snap => {
    ...
    if (data.assist_event && data.assist_event.ts > lastAssistEventTs) {
      ...
      if (!isHandlingSnapshot) { isHandlingSnapshot = true; await showAssistAnimation(); isHandlingSnapshot = false }
    }

    if (data.sync_event && data.sync_event.ts > lastSyncEventTs) {
      lastSyncEventTs = data.sync_event.ts
      if (!isHandlingSnapshot) { isHandlingSnapshot = true; await showSyncAnimation(); isHandlingSnapshot = false }
    }
    ...
    if (!isSpectator) {
      ...
      if (myTurn && !actionDone) {
        ...
        if (myEntry.every(p => p.hp <= 0)) {
          actionDone = true; await doSkipTurn(); return
        }
      }
...
onAuthStateChanged(auth, async user => {
  ...
  // 관전자 or 새로고침(intro_done)이면 인트로 스킵
  if (isSpectator || data?.intro_done) {
    await skipIntro(roomRef)
  } else {
    initIntro(roomRef, mySlot)
  }
